...
namespace JsonKnownTypes
{
    public static class JsonKnownTypesSettingsManager
    {
        ...
        internal static JsonKnownTypesSettings GetSettings<T>()
        {
            ...
            var typeSettings = new JsonKnownTypesSettings(discriminatorSettings.DiscriminatorFieldName);
            ...
            {
                typeSettings.AddAutoGenerated(allTypes);
            }
            else if (!allTypes.All(typeSettings.Contains))
            {
                var missingTypes = allTypes.Where(x => !typeSettings.Contains(x)).Select(x => x.Name);
                
                throw new JsonKnownTypesException($"Dictionary does not contain all classes from { nameof(T) } type hierarchy." +
